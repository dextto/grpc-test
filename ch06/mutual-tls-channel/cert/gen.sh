...
openssl req -newkey rsa:4096 -nodes -keyout client.key -out client.csr -subj "/C=KR/ST=Busan/L=Suyoung/O=Modusign Client/OU=Computer/CN=*.modisign.co.kr/emailAddress=client@modusign.co.kr"

# 5. Use CA's private key to sign client's CSR and get back the signed certificate
openssl x509 -req -in client.csr -days 60 -CA ca.crt -CAkey ca.key -CAcreateserial -out client.crt -extfile client-ext.cnf
